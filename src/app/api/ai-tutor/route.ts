...
export async function POST(request: NextRequest) {
  ...
  if (await isRateLimited(getRateLimitKey(request), usageContext)) {
    return NextResponse.json(
      {
        error:
          "ส่งคำถามถี่เกินไป กรุณารอสักครู่แล้วลองถาม AI ไออุ่นใหม่อีกครั้งครับ",
      },
      { status: 429 }
    );
  }
  ...
  const instructions = [
    "คุณคือ AI ไออุ่น เพื่อนคู่คิดและผู้ช่วยการเรียนรู้ของนักเรียนไทยในเว็บ SciSiam Virtual Lab",
